...
In this section we present the estimation of the diagonal S-matrix elements for representative cases of each difficulty class.\begin{enumerate}
	\item Conditioning an abelian anyon: $S(\Sigma_m, \Sigma_m) = 0.969(6)e^{i\pi 0.004(2)}$. The total compiled circuit depth is 23 with 0 Toffoli gates.
	\item Conditioning a $m$- or $mr$-dyon with $\chi(r^2) = 1$: $S(\Psi_m, \Psi_m) = 0.89(1)e^{i\pi 0.004(4)}$. The total compiled circuit depth is 58 with 2 Toffoli gates.
	\item Conditioning a $r$-dyon with $\chi(r^2) = 1$: $S(\Psi_r, \Psi_r) = 0.82(1)e^{i\pi 0.001(4)}$. The total compiled circuit depth is 77 with 4 Toffoli gates.
	\item Conditioning a non-abelian charge: $S(\Sigma_\epsilon, \Sigma_\epsilon) = 0.123(8)e^{i\pi 0.01(2)}$. The total compiled circuit depth is 125 with 4 Toffoli gates.
	\item Conditioning a $m$- or $mr$-dyon with $\chi(r^2) = -1$: $S(\Phi_m, \Phi_m) = 0.19(2)e^{i\pi 0.04(4)}$. The total compiled circuit depth is 133 with 4 Toffoli gates.
	\item Conditioning a $r$-dyon with $\chi(r^2) = -1$ (semions): $S(\Phi_r, \Phi_r) = 0.06(1)e^{i\pi 1.0(3)}$. The total compiled circuit depth is 171 with 6 Toffoli gates.
\end{enumerate}
...
In the list above we assume that the first element is the being conditioned and we way cycle through other labels without significant increase in the signal-to-noise ratio. This means that the difficulty is set by the row of the S-matrix, in this framing, and one can use the symmetry of the S-matrix to pick the better of the two interference procedures. For example for measuring $S(\Phi_r, \Sigma_m)$ one would condition the $\Sigma_m$ ribbon and not the semion.

\jovan{(Maybe) A figure schematically showing the various S-matrix elements difficulty.}
...
Arguably, where we loose out the most is with the reduced charge measurement. The group $S_3$ has only two proper subgroups, $H_r = \mathbb{Z}_3^r$ and $H_m = \mathbb{Z}_2^m$.

In general, sets of group elements onto which we reduce the domain of the controlled multiply maps such that the circuit simplifies significantly is scarce for such a small group. 
...
Table \ref{tab:red_ch_S3} shows the partial orthogonality of character tables of $S_3$ and it's two proper subgroups. The fact that given the results of both $H_m$- and $H_r$-reduced charge measurements we can uniquely determine the charge still holds for this group as well. 

\textbf{Summary.}
The computational universality of the $D(S_3)$ theory complemented with measurements comes with a great cost of significantly longer elementary circuit of our protocols. 

The elementary circuits are about ten times deeper once compiled into native gates that the Sycamore can perform, neglecting the SWAPs needed to place the relevant qubits into appropriate proximity to one another.

This comes out, mainly, from a unnatural way we encode the group elements, unnatural in a way that doesn't respect the group structure ($\mathbb{Z}_2 \ltimes \mathbb{Z}_3$). A more suitable encoding is achieved by introducing qutrits alongside the full one-qutrit control and ability to condition arbitrary one-qutrit gates. In this encoding all of the elementary circuits become even shallower than in the case of $D_4$. 

However, assuming such a resource is unreasonable, hence, using this protocols to study the braiding properties of the $D(S_3)$ anyons is beyond what NISQ can do.
