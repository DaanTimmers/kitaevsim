...
\jovan{I do not know where you found this claim, according to \cite{weber} the readout times (4us) are hundred times longer than duration of gates (35ns)?}
...
\caro{steve: be more precise about here bc important}\jovan{I do not know what would I add here?}
...
\caro{Steve's comment: when is it true that measuring subgroups is enough to unambiguously determine charge}\jovan{Sadly, we never did this analysis. I suspect the partial orthogonality claim is true for any group, but the solvability of the group may imply that the $H$-controlled multiplication is drastically simpler to perform.}
...
While the general expression for the $N_{ab}^c$ is cumbersome\footnote{and can be found in Appendix \ref{app:reps} for the case of $D(D_4)$ anyons \jovan{The original App was removed.}}, for pure charge anyons $(\{e\}, \chi_i)$ it readily reduces to the well-known  decomposition of tensors products of group irreps into direct sums of irreps $\chi_i\otimes\chi_j = \bigoplus_k n^k_{ij} \chi_k$.
...
When encoding the internal space of the anyon, which we need for the ribbon operator protocol, we note that the basis $\ket{\nu} = \ket{c, i}$, contains a group element $c\in C$ restricted to a conjugacy class. All the non-trivial conjugacy classes contain just two elements and can be encoded with only one qubit. Moreover, the entire internal space of the anyons with non-trivial flux is spanned by only one qubit e.g. for $\Phi_m = (\mathcal{C}_m, (1,-1))$ anyons we have
\begin{equation}
	\ket{c, 0} \equiv \ket{a}_{r^2} \iff c = m^1 r^0(r^2)^a \in \{m , mr^2\}.\footnote{In the generalised conjugation circuits we use a different set of labels for this qubit: $\text{aux}$ (any auxiliary bit), $a_b$ (forward ancilla) or $a_f$ (backwards ancilla), depending on the context.\jovan{Maybe rephrase?}} 
\end{equation}
...
\begin{figure}
    ...
    \caption{The two braiding protocols, differing only in order of exchange of two anyons. C stands for creation, M for measurements. Protocol a) will have 4 fusion outcomes, while protocol b) can only produce vacuum.\caro{change C to M in figure}\jovan{Looks like you did it?}}
...
In fact, both can be measured easily by the protocol in Figure~\ref{fig:cond_ex} since anyons $0$ and $\tilde{0}$ are abelian and their ribbon operator only have single qubit gates. Similar protocol was used to measure the $S$-matrix in the case of the Toric code\cite{Satzinger_2021}\jovan{More refs?}.
...
\caption{The interference protocols used for phase sensitive measurement of the (normalized) S-matrix elements  $\tilde{S}(a,b) = \frac{|D_4|}{d_a d_b}S(a,b)$.\caro{steve: why not call $0\oplus 0$ just $0$ and $0 \oplus \tilde 0 $ just $\tilde 0$}\jovan{It is subtle, but our protocol differentiates these two sets of labels, so they are not the same.}}
\label{fig:S-mat}
\end{figure*}
...
\jovan{Okay, I think this is something to add in the Outlooks section.}
...
\caro{is it clear that the $\pm$ for 4 bins refers to max/min}\jovan{This part of the data is mostly qualitative anyways.}
...
% counting $495 \pm 10$ on the braiding ladder and $xxx \pm xx$ on the small planar graph since 
 %In both cases the biggest noise peak accounts for a number of shots of the same order as in $H_{mr}$-reduced charge measurement. This is expected since the noise is uncorrelated with what measurement protocol we chose to implement.

%The signals doubles because both of the pairs of charges $(0, \Sigma_m)$ and $(\tilde 0, \tilde \Sigma_m)$ are indistinguishable with $H_m$-reduced charge measurement.

%Given these outcomes for the two sets of measurement, the trivial flux components and the number of peaks in the two reduced charge measurements, the only compatible charge content is the quadruple $(0, \Sigma_m, \tilde 0, \tilde \Sigma_m)$.
...
\end{equation}
\caro{there was a comment by steve on $\epsilon$ being wrong but I think they are right}\jovan{They are.}
...
In Fig.~\ref{fig:Smat_diff} we show the $S$-matrix with all its entries color coded according to the difficulty class. In the list below we present the circuit depths and numerical simulation results for the diagonal S-matrix elements for representative cases of each difficulty class. For all protocols \jovan{add:(except for 1. and 4.)} we chose to condition ribbon vs $0\oplus \tilde 0$ rather than ribbon vs vacuum.
...
\begin{figure*}
	...
	\caption{The S-matrix of the $D(D_4)$ theory. The color shading represents the difficulty to observe the values experimentally, where green to red denotes increasing difficulty. \jovan{Rewritten:The entries highlighted in bold face and blue had their values numerically obtained by simulating the phase-sentitive measurement protocols and are discussed in the text.}}
	\label{fig:Smat_diff}
\end{figure*}
...
The anyons of this theory alongside their braiding properties have been completely studied \caro{in Ref. \cite{Cui_2015}}\jovan{added}. In this section we just note that there are two abelian, four two dimensional and two three dimensional anyons.
...
%$U_{CM}^{(4)}: \ket{g,h} \rightarrow \ket{g, hg^{-1}}$, 
for $g \in G$ (unrestricted), $g$ restricted to subgroups $H$ and $g$ restricted to conjugacy classes $C$.\caro{fix label for conjugacy class qubit}\jovan{Hopefully adequately addressed in the main text.}}
    \label{fig:CM}
\end{figure}
...
\textbf{Miscellaneous. \caro{propose to omit}\jovan{Fair point.}} In Section~\ref{sec:D4_double} we have already listed all the decoupling circuits used in the reduced charge measurement protocol in the case of $D(D_4)$. Some other useful circuits are shown in Figure~\ref{fig:Misc_cirq}.
