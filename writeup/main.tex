...
The biggest determining factor for the difficulty is weather for the representation in question $(C, \chi)$ obeys $\chi(r^2) = \chi(e)$. If this is not the case, points 4., 5. and 6., the representation $A^{(g)}$ is faithful hence very difficult to compile since all group element must be included in the circuit which implies many SWAPs for the surface code layout of qubits. 

The second is the number of Toffoli gates in the Controlled Multiplication and Generalised Conjugation circuits. The semions are difficult in both regards, hence our interference protocol so poorly estimates the relevant matrix elements. Noting that the phases of the elements are much better estimated than the magnitudes.
