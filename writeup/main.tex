...
%\footnote{In the controlled multiplication circuits we label this bit as $r^2$, i.e. $\ket{a}_{r^2}$, because that is what they represent in the group multiplication. In the generalised conjugation we use either the label $\text{aux}$ (any auxiliary bit), $a_b$ (forward ancilla) or $a_f$ (backwards ancilla), depending on the context.} 
...
\end{equation*}
    ...
    \caption{Circuits for generalised conjugation. The first three qubits encode the physical, group valued, gauge field $\ket{g}$. The last qubit encodes the auxiliary qubit representing the internal state of the (two-dimensional) anyon $\ket{c}$. Left: The conjugation unitary for a pure flux $\Psi_m$ $\ket{c}\ket{g} \rightarrow \ket{gcg^{-1}}\ket{g}$. Right: The generalised conjugation unitary for the dyon $\tilde{\Phi}_m$: $\ket{c}\ket{g} \rightarrow \Gamma(g)\ket{gcg^{-1}}\ket{g}$, where the representation 'matrix' $\Gamma(g) \in U(1)$ and  $S = \text{diag}(1, i)$.}
...
\begin{figure}
    ...
    \caption{The two braiding protocols, differing only in order of exchange of two anyons. C stands for creation, M for measurements. Protocol a) will have 4 fusion outcomes, while protocol b) can only produce vacuum.\caro{change A to M in figure (M for measurement)}}
