...
The biggest determining factor for the difficulty is weather for the representation in question $(C, \chi)$ obeys $\chi(r^2) = \chi(e)$. If this is not the case, points 4., 5. and 6., the representation $A^{(g)}$ is faithful hence very difficult to compile since all group element must be included in the circuit which requires many SWAP gates. 
