...
The full measurement protocol, in general, then consist of performing a $H$-partial charge measurement and reading-off the flux $f$ on the same site. 
...
\end{equation*}
    ...
    \caption{Circuits for controlled group multiplication $\ket{g,h} \rightarrow \ket{g,gh}$. Left: Both $g$ (first three qubits) and $h$ are unrestricted, i.e., $g,h\in G$. Center: $g \in H_{m}$ is encoded by just the first two qubits. Right: $g \in C_m$ is encoded by just the first qubit.}
    \label{fig:restMult}
\end{figure}
...
\begin{figure}
	...
	\caption{Ribbon operators and qubit layout for the fusion of two $\Psi_m$ anyons on the braiding ladder (top) and a small planar graph (bottom). Purple circle mark the plaquette on which we perform the $H_{mr}$-partial charge measurement. Red circles mark the vertex on which we measure the flux.}
...
\begin{figure}
	...
	\caption{Ribbon operators for the two braiding experiments. On the last step we also label around which plaquette we perform the $H_{mr}$-partial charge measurement. Note that the lattice is embedded into a sphere, so the outside plaquette that we labelled twice for clarity should be identified.}
...
\begin{subfigure}{0.47\textwidth}
    ...
    \caption{The partial charge-flux measurement after fusing two $\Psi_m$ pure fluxes on the site of fusion. The partial charge was done with $H_{mr}$ subgroup, and the geometry was that of the braiding ladder, Figure \ref{fig:latticeGS}.}
    \label{fig:fusion_glass}
\end{subfigure}\hfill
\begin{subfigure}{0.47\textwidth}
    \includegraphics[width=\linewidth]{Figures/fusion_on_basketball.png}
    \caption{The partial charge-flux measurement after fusing two $\Psi_m$ pure fluxes on the site of fusion. The partial charge was done with $H_{mr}$ subgroup, and the geometry was that of the small two-dimensional graph, Figure \ref{fig:basketball}.}
...
\end{equation*}
	...
	\caption{The controlled multiplication circuits implementing the maps $\{U_{CM}^{(1)}, U_{CM}^{(2)}, U_{CM}^{(3)}, U_{CM}^{(4)}\}$ from left to right. The first three qubits encode the control group element and the second three the target group element.}
	\label{fig:CM_Unres}
\end{figure}
...
\end{equation*}
    ...
    \caption{The controlled multiplication circuits implementing the maps $\{U_{CM}^{(1)} = U_{CM}^{(3)}, U_{CM}^{(2)} = U_{CM}^{(4)}\}$ from left to right. The first three qubits encode the control group element and the second three the target group element. The first element is restricted to be in the $H_m$ subgroup.}
    \label{fig:CM_Hm}
\end{figure}
...
\end{equation*}
    ...
    \caption{The controlled multiplication circuits implementing the maps $\{U_{CM}^{(1)} = U_{CM}^{(3)}, U_{CM}^{(2)} = U_{CM}^{(4)}\}$ from left to right. The first three qubits encode the control group element and the second three the target group element. The first element is restricted to be in the $H_{mr}$ subgroup.}
    \label{fig:CM_Hmr}
\end{figure}
...
\end{equation*}
    ...
    \caption{The controlled multiplication circuits implementing the maps $\{U_{CM}^{(1)}, U_{CM}^{(2)}, U_{CM}^{(3)},  U_{CM}^{(4)}\}$ from left to right. The first three qubits encode the control group element and the second three the target group element. The first element is restricted to be in the $H_{r}$ subgroup.}
    \label{fig:CM_Hr}
\end{figure}
...
\end{equation*}
    ...
    \caption{The controlled multiplication circuits implementing the maps $\{U_{CM}^{(1)} = U_{CM}^{(3)}, U_{CM}^{(2)} = U_{CM}^{(4)}\}$ from left to right. The first three qubits encode the control group element and the second three the target group element. The first element is restricted to be in the $C_m$ conjugacy class.}
    \label{fig:CM_Cm}
\end{figure}
...
\end{equation*}
    ...
    \caption{The controlled multiplication circuits implementing the maps $\{U_{CM}^{(1)} = U_{CM}^{(3)}, U_{CM}^{(2)} = U_{CM}^{(4)}\}$ from left to right. The first three qubits encode the control group element and the second three the target group element. The first element is restricted to be in the $C_{mr}$ conjugacy class.}
    \label{fig:CM_Cmr}
\end{figure}
...
\end{equation*}
    ...
    \caption{The controlled multiplication circuits implementing the maps $\{U_{CM}^{(1)}, U_{CM}^{(2)}, U_{CM}^{(3)},  U_{CM}^{(4)}\}$ from left to right. The first three qubits encode the control group element and the second three the target group element. The first element is restricted to be in the $C_{r}$ conjugacy class.}
    \label{fig:CM_Cr}
\end{figure}
...
Another building block of our $(C, \chi)$-ribbon operator application protocol is the what we call generalised conjugation map, $U_{GC}\ket{g}\ket{c,i} = \ket{g}\ket{gcg^{-1}}\Gamma_c\ket{i}$. Written like this it is clear why we call it generalised conjugation, however, it is more naturally defined via the algebra representation matrices $U_{GC}\ket{g}\ket{\alpha} = \ket{g}A^T(g)\ket{\alpha}$, where $\alpha$ is the combined index going over both the conjugacy class and the $\chi$-representation index. These matrices in the case of $D(D_4)$ are shown in Appendix~\ref{app:reps}.
...
\end{equation*}
    ...
    \caption{(Left) The inversion circuit, $\ket{g} \rightarrow \ket{g^{-1}}$, used when reversing the arrow direction of the edges. (Right) The basis change circuit, $\ket{g}_{mRR^2} \rightarrow \ket{g}_{(mR)RR^2}$, can be used to simplify $mr$-based ribbon application protocol by making them analogous to $m$-ribbon application protocols.}
    \label{fig:Misc_cirq}
\end{figure}
