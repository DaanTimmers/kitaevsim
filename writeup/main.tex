...
\end{equation}
 ...
 \begin{figure}
 	...
 	\caption{(Left) The diagram definition of the linking matrix elements between anyons $a$ and $b$ and its evaluation in the case of the quantum double model. $\sigma_{ii+1}$ represent the action of the braid group generator, the exchange of $i^{\text{th}}$ and $(i+1)^{\text{st}}$ anyon, on the representation space $a \otimes a \otimes b \otimes b$. $P_{xx}^0$ are projectors from the representation space $x \otimes x$ onto the trivial sector and $\phi_{xx}^0$ are functions whose inverses are these projectors. (Right) An example of ribbon crossing that arrises while braiding or annihilating the anyons. The two legs shared between the ribbons are marked in bold. Note that for each leg they share the ribbons have the opposite type of elementary triangle touching it; also that for each ribbon the triangles that touch the two shared legs are of opposite type.}
 	\label{fig:S_mat_def}
 \end{figure}
 
 In Figure \ref{fig:S_mat_def} (Left) we also shown how to evaluate the matrix element in the case of quantum double model, the only additional information we need to evaluate it is also provided by the definition of the quantum double algebra.
The action of the exchange of anyons on the product space is  given bu
\begin{equation}
	\begin{split}
...
The crossing ribbons share two legs and on these two legs the type of their elementary triangles are opposite, also for each ribbon the type of the elementary triangles
