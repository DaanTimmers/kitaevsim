...
If the ribbons are closed \jovan{and contractible}, they leave the ground state unchanged, since there are no ends. Moreover, they span all (loop) operators that leave the ground state invariant, it is in that way that the quantum double ground state knows about the quasiparticle spectrum.
...
\begin{figure}
    ...
    \caption{An example of a ribbon, $R = \{t_1, t_2, \ldots, t_8\}$, between site $s_1$ and $s_2$. The ribbon is made up from six Type I elementary triangles and two Type II elementary triangles, they are labelled. \jovan{Make it more regular.}}
