...
\textbf{Ground State.} It is not hard to verify that all terms in the Hamiltonian commute, Hence we can diagonalize it term-by-term. One can show that the leftover degeneracy depends only on the genus of the surface the graph is embedded in\cite{Kitaev_2003, cui2018topological}. However, we will work on a sphere-topology, where the ground state is unique.
...
\textbf{Excitations.} The ground state is stabilized\footnote{Meaning, it is the $+1$ eigenstate.} by the projectors in Eq.~\eqref{eqn:ham}. Together with the property $A_p^{(g_1)}A_p^{(g_2)} = A_p^{(g_1 g_2)}$ this implies the stronger condition
\begin{equation}
\begin{split}
...
Note, that in the case of abelian groups, in particular the toric code, the conjugacy classes are trivial and identical to the group elements themselves. Their center is $G$, which has $|G|$ one-dimensional representations. Hence, the irreducible representations factorize as indicated above and the anyons are Abelian. 
...
How this manifests physically is that if we have anyons $a$ and $b$ at two sites and we fuse them and measure the topological charge, anyon label, at the site of the fusion we may get any label $c$ for which $N_{ab}^c \neq 0$.

The gauge group being Abelian implies $|C|=1$ for all conjugacy classes and $\text{dim}(\chi) = 1$ for all group representations, i.e. all \emph{algebra} representations are one-dimensional, $d = 1$. There is no degeneracy once the charge content of a gauge field is specified and the fusion is unambigous. 
...
For Non-Abelian gauge groups, there are always one-dimensional representations associated with Abelian anyons of the Non-Abelian quantum double model. Majority of representations, however, are have dimension larger than one and are associated with the Non-Abelian anyons. It is the Non-Abelian anyons that contribute to the dimension of the degenerate subspace with a certain charge content. 

As an example, let us take the charge content of the gauge filed to be four Non-Abelian anyons of the same type $a$ occupying four sites and look at the form of the degenerate subspace $\mathcal{H_{\text{deg.}}}}$.
...
In the following text, we will explain how to create and move the anyons, i.e. the excitations we have classified above.

Anyons are always created in pairs from the vacuum. 
...
The ribbon operators are not unitary in general. If the anyons have a dimension $d$ larger than one, the operators that create and manipulate them are non-local projectors. This is a problem when compiling unitary quantum circuits for braiding Non-Abelian anyons, we will need measurements. 
...
As we mentioned, the pair of anyons define a $d^2$-dimensional degenerate subspace of the gauge field Hilbert space that encodes the outcomes of their fusion. This encoding is increasingly non-local as we separate the anyons, this being the main idea of topological quantum computation with anyons.


We, however, require storing this information locally since we need to do measurements on this space to create and move anyons. This is done by keeping one auxiliary qudit per anyon/end of the ribbon.
...
\section{$D_4$-Lattice Gauge Theory}
...
This decomposition is not unique, $D_4 = \mathbb{Z}_2^m\ltimes(\mathbb{Z}_2^{r^2}\times\mathbb{Z}^{mr}_2)$ is also a valid decomposition.

Note, the superscripts in $\mathbb{Z}_n^x$ label the group generator, $x^n = e$.

This group has five conjugacy classes. The conjugacy classes alongside their centres are listed in Table
...
\centering    \begin{tabular}{|c | c|}\hline
         ...
         \hline 
    \end{tabular}
    \caption{The conjugacy classes of $D_4$ alongside their centres}\label{tab:char}
\end{table}
