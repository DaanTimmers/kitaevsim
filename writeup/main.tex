...
\title{non-Abelian Braiding of Lattice Gauge Theory excitations on NISQ hardware}
...
%
%
%In Kitaev's toric code the topological ground state degeneracy is used to protect quantum information against noise. The topological phase needed for that is relatively simple, its excitations are Abelian and 
%
%as is done in Kitaev's toric code, the idea of performing computations via the braiding of anyons is referred to 
%
%
% \emph{non-Abelian anyons}
%
%
%, particles that change the state of the system by a unitary operation instead of just a phase
%
%
%However, almost half a century later unambiguous experimental evidence for demonstrating non-Abelian anyons in 'natural' physical systems is still scarce.
...
Using this framework, we examine a couple of braiding experiments. The question we address is the plausibility of detecting the non-Abelian braiding signatures given the levels of noise of current digital quantum simulators, Google's Sycamore chip in particular.
...
\begin{figure}
	...
	\caption{(Top) The ribbon operators involved in two braiding experiments. On the last step we also label around which plaquette we perform the $H_{mr}$-reduced charge measurement. Note that we are on a sphere so the outside plaquette that we labelled twice for clarity should be identified, this is best seen in the diagram for $\sigma_{12}$ exchange. (Bottom) The circuit layout on the Sycamore chip we used to simulate the braiding experiment.}
...
\emph{Fusion.} 
On both lattices we see the signatures of four fusion outcomes
$$\Psi_m \otimes \Psi_m = 0 \oplus \tilde 0 \oplus \Sigma_m \oplus \tilde{\Sigma}_m.$$
with the difference coming as the larger background noise in the case of the small planar graph, due to a significantly deeper circuit due to the ground state preparation.

The number of runs for the fusion on the braiding ladder was $16000$. The expected post-selection probability for the two ribbons is $1/16$, while the actual number of successes was $1807$, due to the circuit noise and measurement readout bias. The four main bins count $270(16)$ with the biggest noise peak being $77(8)$. 

The number of runs for the fusion on the small planar graph was $XXX$. The expected post-selection probability for the two ribbons is $1/16$, while the actual number of successes was $XXX$, due to the circuit noise and measurement readout bias. The four main bins count $XXX$ with the biggest noise peak being $XXX$. \jovan{I lost the data, rerunning the experiment.}

\emph{Braiding.}
Looking at the results of the charge measurement for the two braiding protocols we clearly see what we expected. The first braiding protocol leaves multiple fusion outcomes at the site of stitching while the second braiding protocol leaves it in the vacuum.

The number of runs for both protocol was $16000$. The expected post-selection probabilities for the two protocols are $1/16$ for $\sigma_{12}\sigma_{23}$ and $1/2$ for $\sigma_{23}\sigma_{12}$, while the actual number of successes was $1977$ for $\sigma_{12}\sigma_{23}$ and $5036$ for $\sigma_{23}\sigma_{12}$, due to the circuit noise and measurement readout bias.
