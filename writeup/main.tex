...
In this section, we provide the specifications of the simulation whose results we presented above.

The layout of the qubits on the Sycamore chip is shown in Figure \ref{fig:intef_setup}.
\begin{figure*}
	\centering
	\includegraphics[width=0.75\linewidth]{Figures/intef_setup.pdf}
	\caption{(Left) The qubit layout of the interference protocol simulation done via Google Colab platform. The chip simulated is the Google's Sycamore chip and the layout was chosen to occupy a part of the chip with low noise two qubit connections, also this layout reduces the overall number of two qubit gates used int the final circuit. (Right) The ribbons operators applied in the simulation.}
...
There are sixteen kinds of elementary triangles, which can be understood as coming from four binary descriptors:\begin{enumerate}
	\item The aforementioned triangle type, i.e. does it cross or follow the edge it touches,
	...
	\item The edge it touches can be oriented in two direction.
\end{enumerate}
