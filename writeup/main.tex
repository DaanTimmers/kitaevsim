...
All other anyons are called dyons. They violate vertex and plaquette terms simultaneously, meaning they have a flux component associated with a vertex of the site $(v,p)$ and a charge component associated with its plaquette.

\jovan{Redo the Fusion Section}

\textbf{Fusion Algebra.} To understand the distinction between the excitations of the Abelian and Non-Abelian gauge fields we will focus on the physical meaning of the internal dimension of the anyon.

If the anyon has a label $(C, \chi)$ it is associated with that irreducible representation. This means that if this anyon is occupying a site $s$ the plaquette and vertex terms of that site will transform the state in accordance with that representation.

The representation acts on a vector space of dimension $d = \text{dim}(C, \chi) = |C|\text{dim}(\chi)$. Hence if we specify that gauge field has a particular excitation at a particular site, this description must be degenerate in order to allow vertex and plaquette terms to be represented accordingly. This degeneracy is exactly $d$-fold and can be associated with a spin-like local degree of freedom of the anyon itself, internal space being spanned by the basis defined in the previous section, $\ket{c, i}$.

Moreover if we completely specify the charge content of the gauge field, i.e  anyon labels at every site, this will not uniquely determine the gauge field state, but only a degenerate space of dimension
\begin{equation}
	D = |\mathcal{H_{\text{deg.}}}| = \prod_s d_s.
\end{equation}
Moreover we can then associate the degenerate space $\mathcal{H_\text{fusion}}$ with the product of internal spaces of anyons at each site $$\mathcal{H_{\text{fusion}}} \equiv \bigotimes_s (C_s, \chi_s).$$

This description, however, there is an algebra associated with the tensor product of representations, analogous to the Glebsch-Gordan (GC) decomposition of tensor product of representations into tensor sum of irreducible representations. In fact, for pure charges this algebra is the GC decomposition
\begin{equation}
\begin{split}
...
\end{equation}
with $a$, $b$ and $c$ going over a set of all anyon labels $(C, \chi)$. See Appendix \ref{app:fusion} for explicit evaluation of the algebra/decomposition coefficients $N_{ab}^c$ for the representations of the quantum double.

How this manifests physically is that if we have anyons $a$ and $b$ at two sites and we fuse them and measure the topological charge, anyon label, at the site of fusion we may get any label $c$ for which $N_{ab}^c \neq 0$.

If the gauge group is Abelian that implies $|C|=1$ for all conjugacy classes and $\text{dim}(\chi) = 1$ for all group representations, i.e. all \emph{algebra} representations are one-dimensional, $d = 1$. There is no degeneracy once the charge content of a gauge field is fixed.
Looking at the dimensions of the LHS and RHS of Eq.~\eqref{eqn:fuse}, we see that for every $a$ and $b$ there is only one $c$ for which $N_{ab}^c$ is nonzero, i.e. one. The fusion of anyons is unambiguous. This fact, for example, implies that splitting of charge and flux components of an Abelian dyon is well-defined since we can always recombine it and get the charge we dyon we started with. This is not the case for Non-Abelian dyons, $d>1$.

Let us now use the algebra of Eq.~\eqref{eqn:fuse} to simplify the form of the $\mathcal{H_{\text{deg.}}}}$. For this example, we will take the charge content to be four anyon of the same type $a$ occupying four sites
\begin{equation}
	\mathcal{H}_{\text{deg.}} = \bigoplus_{c, d, e} N_{aa}^c N_{ca}^d N_{da}^ee,\label{eqn:decomp} 
\end{equation}
let us also assume that we have measured zero total topological charge, i.e. $e$ is fixed to be $(\{e\}, \mathbb{1}) = 0$.

This equation informs a choice of basis that has nothing to do with the local spin-like variables and is spanned by all fusion outcomes $(c, d)$ for which none of the algebra coefficients in Eq.~\eqref{eqn:decomp} vanish, $$\mathcal{H}_{\text{deg.}} = \mathcal{H}_{\text{fusion}} = \text{span}\{\ket{c,d; e, 0}\},$$
hence this space is also called the fusion space, and this basis, the fusion tree basis. The second pair of indices label the one state in the vacuum representation and can be dropped.
Note that addition of sites occupied by vacuum does nothing since vacuum fuses trivially with everything, $a\otimes0 = a$.

\jovan{assumed def in the intro}
Braiding anyons acts on this degenerate space, hence for braiding to have any computational power, the degeneracy must be non-trivial and anyons Non-abelian. See Appendix \ref{app:braid} for the treatment of braiding of quantum double anyons in both local and fusion basis.
