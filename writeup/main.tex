...
\section{Post-selection and one-qubit marginals}\label{app:DATA_Anl}

\textbf{Single Qubit Tomography.} In the following text, we discuss the uncertainty estimations used in our analysis of the interference protocols.

The underlying probability distribution governing the outcome of $N$ single qubit readouts is a biased binomial distribution, $P(n_s) = {N \choose n_s} p_s^{n_s} p_{\bar{s}}^{N-n_s}$, where $n_s$ counts the number of times an outcome $s \in \{0,1\}$($\bar{s} = 1-s$) was achieved after the measurement. 

From a set of $N$ shots we want to estimate a function we called polarisation $P = \frac{p_0 - p_1}{p_0 + p_1}$, that is, by our assumption, related to the parameters defining the single qubit state and read-out characteristics of the machine, $P = P(\vec{b}) = (1-2\bar\epsilon)\vec{r}\cdot\vec{b} + \Delta\epsilon$. 

The unbiased estimator for this quantity is naturally $\hat P = \frac{n_0 - n_1}{n_0 + n_1}$, with $$\text{Mean}(\hat P) =P\text{ and }\text{Var}(\hat{P}) = N\sigma_{P}^2 = P(1-P),$$ where $\sigma_{P}$ is then the error in determining this mean. This error is determines the error bars in Figures~\ref{fig:S_res} and ~\ref{fig:t_mat_results}.

Given the estimates for the function $P(\vec{b})$ for different measurement bases, $\vec{b}$, we extract the relevant quantities $(\vec{r}/|\vec{r}|, \Delta\epsilon, |(1-2\bar\epsilon)\vec{r}|)$ by fitting.

Firstly for the $\phi$-scan we fit the function $f(\phi) = A\cos(\phi - \phi_{\text{max}})+B$, mainly to extract the angle which maximised the polarisation $\phi_{\text{max}}$. Then, fixing $\phi = \phi_{\text{max}}$, we do the $\theta$-scan and to it we fit the more informative function $g(\theta) = |(1-2\bar\epsilon)\vec{r}|\cos(\theta - \theta_{\text{max}}) + \Delta\epsilon$. The two angles define the direction of the Bloch's vector, while the other parameters are gotten from entirely from the second fit.

The two angles define the braiding amplitude we want to extract from the interference protocol $$\mathcal{A} = \left(\sqrt{\frac{1-\cos{\theta_\text{max}}}{1+\cos{\theta_\text{max}}}}\pm \frac{\sigma_{\theta_\text{max}}}{|1 +\cos{\theta_\text{max}|}}\right)e^{i(\phi_\text{max}\pm \sigma_{\phi_\text{max}})},$$
where the angles and their uncertainty are obtained by the usual $\chi^2$-fitting method.
 
 
\textbf{Biased Post-Selection.} We investigate the effect of post selecting on biased measurement outcomes for the paradigmatic example of two qubits. We consider a general two-qubit density matrix
\begin{equation}
