...
\textbf{Supplementary Measurements.} In the analysis done above we rely on the knowledge of the fusion outcome to label the peaks in Figure~\ref{fig:red_charge_res}. 

However, we can still determine what charges do the peaks correspond to even if we don't rely on the knowledge of the fusion algebra. This is done by performing reduced charge measurements over other subgroup, as explained in Section \ref{sec:redchmmt}.

Repeating the protocol with a $H_m$-reduced charge measurement we see only two peaks counting $495 \pm 10$ on the braiding ladder and $xxx \pm xx$ on the small planar graph. In both cases the biggest noise peak accounts for a number of shot of the same order as in $H_{mr}$-reduced charge measurement. This is expected since the noise is uncorrelated with what measurement protocol we chose to implement.
...
Similar analysis can be carried out generally.
...
The second factor to consider is the number of Toffoli gates in the controlled multiplication and generalised conjugation circuits. The semions \caro{XY} are difficult in both regards.


In the list below we present the circuit depths and numerical simulation results for the diagonal S-matrix elements for representative cases of each difficulty class. 
...
\caro{Having access to qutrits again trivialises both problems.}
...
\pgfplotstabletypeset[dec sep align,
   fixed zerofill,
   precision=0,
   col sep=space]{S_ranked_diff.csv}
