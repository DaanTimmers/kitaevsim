...
\begin{equation}
	\mathcal{H}_{\text{deg.}} = \bigoplus_{c, d, e} N_{aa}^c N_{ca}^d N_{da}^ee, 
\end{equation}
let us also assume that we have measured zero total topological charge, i.e. $e$ is fixed to be $(\{e\}, \mathbb{1}) = 0$.

This equation informs a choice of basis that has nothing to do with the local spin-like variables and is spanned by all fusion outcomes $(c, d)$ for which none of the above mentioned algebra coefficient vanish, $$\mathcal{H}_{\text{deg.}} = \mathcal{H}_{\text{fusion}} = \text{span}\{\ket{c,d}\},$$
hence this space is also called the fusion space, and this basis, fusion tree basis.
